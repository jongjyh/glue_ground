...
class L0Dense(Module):
    ...
    def __init__(self, in_features, out_features, bias=True, weight_decay=0., droprate_init=0.5, temperature=.5,
                 lamba=2/3., local_rep=False, **kwargs):
        """
        ...
        self.qz_loga = Parameter(torch.empty((in_features,)))
    ...
    def _reg_w(self):
        ...
        logpw_col = torch.sum(- (.5 * self.prior_prec * self.weight.pow(2)) - self.lamba, 0)
    ...
    def count_expected_flops_and_l0(self):
        ...
        ppos_mean = ppos/self.in_features 
        expected_flops = (2 * ppos - 1) * self.out_features
        expected_l0 = ppos * self.out_features
        if self.use_bias:
            expected_flops += self.out_features
            expected_l0 += self.out_features
        return expected_flops.detach().item(), expected_l0.detach().item(),ppos_mean.detach().item()
