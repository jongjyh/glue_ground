    ...
    def __init__(
        ...
        for i in range(1, max_nodes):
            for j in range(i):
                ...
                if j == 0:
                    xlists = [
                        OPS[op_name](C_in, C_out, stride, affine,dila, track_running_stats)
                        for op_name in op_names
                    ]
                else:
                    xlists = [
                        OPS[op_name](C_in, C_out, 1, affine,dila, track_running_stats)
                        for op_name in op_names
                    ]
...
if __name__ =='__main__':
    from adapter import op_name
    cell =NAS201SearchCell(768,768,1,6,op_name)
    input = torch.randn((10,32,768))
    
    def get_gumbel_prob(xins):
        while True:
            gumbels = -torch.empty_like(xins).exponential_().log()
            logits = (xins.log_softmax(dim=1) + gumbels) / 0.1
    ...
    arch_parameters = torch.randn((cell.num_edges,len(op_name))).requires_grad_(True) 
    hardwts, index = get_gumbel_prob(arch_parameters)

    cell.num_edges 
    output = cell.forward_gdas_v1(input,hardwts=hardwts,index=index)
    print(output.shape)